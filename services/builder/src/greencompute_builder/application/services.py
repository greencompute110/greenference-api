...
class BuilderService:
    def __init__(
        ...
        self.settings = load_runtime_settings("greencompute-builder")
        ...
        self.metrics = get_metrics_store("greencompute-builder")
    ...
    def _write_context_archive(context_archive_b64: str, archive_name: str) -> tuple[str, int]:
        ...
        context_dir = Path(tempfile.gettempdir()) / "greencompute-builder-contexts"
        context_dir.mkdir(parents=True, exist_ok=True)
        payload = base64.b64decode(context_archive_b64.encode())
        with tempfile.NamedTemporaryFile(
            prefix="greencompute-build-",
            suffix=suffix,
            dir=context_dir,
            delete=False,
        ) as outfile:
